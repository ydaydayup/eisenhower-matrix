...
export function usePinWindow() {
  ...
  const refreshStatus = useCallback(() => {
    if (isClient && typeof window !== 'undefined' && window.electron) {
      try {
        ...
        console.error('[PinWindow] 刷新状态失败:', err);
      }
    }
  }, [isClient]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('[PinWindow] 发送get-pin-status请求失败:', err);
      setError('发送状态请求失败');
    }
    ...
    try {
      ...
      console.error('[PinWindow] 设置旧通道状态监听器失败:', err);
    }
    ...
    try {
      ...
      console.error('[PinWindow] 设置状态同步监听器失败:', err);
      setError('设置状态监听器失败');
    }
  ...
  const togglePin = () => {
    ...
    try {
      ...
      console.error('[PinWindow] 切换钉在桌面状态失败:', err);
      setError('切换状态失败');
    }
  };
  ...
  const setPinStatus = (status: boolean) => {
    ...
    try {
      ...
      console.error('[PinWindow] 设置钉在桌面状态失败:', err);
      setError('设置状态失败');
      ...
      if (isPinned !== newStatus) {
        try {
          ...
          console.error('[PinWindow] 备选方案也失败:', toggleErr);
        }
      }
    }
  };
