// 使用CommonJS语法
const { app, protocol, BrowserWindow, Menu, session, shell, ipcMain, Tray, nativeImage, Notification } = require('electron');
const path = require('path');

// 设置应用名称为"AI提效"
app.setName('AI提效');
app.setAppUserModelId('AI提效');
...
let createHandler;
try {
  const nextElectronRsc = require('next-electron-rsc');
  createHandler = nextElectronRsc.createHandler;
} catch (error) {
  console.error('无法加载 next-electron-rsc 模块:', error.message);
  console.error('将使用备用启动方法');
  createHandler = () => ({ 
...
const { createInterceptor, localhostUrl } = handlerObj;
