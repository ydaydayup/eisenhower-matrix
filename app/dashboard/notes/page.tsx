...
import { PlusCircle, Edit, Trash2, Search, FileText, Clock, Type, Code, Table, Minus, Link as LinkIcon, FileIcon, AlignLeft, Folder, CalendarDays, Hash, Bookmark, AlignJustify, CheckSquare, ListOrdered, List, Check, X, Save } from "lucide-react"
...
const NotesPageClient = () => {
  ...
  const renderNoteItem = (note: Note) => (
    <div
      key={note.id}
      className={`group flex flex-col p-2 rounded-lg cursor-pointer note-item transition-all duration-200 ${
        selectedNote?.id === note.id
          ? 'bg-blue-50/70 dark:bg-blue-900/30 border border-blue-200/50 dark:border-blue-700/50'
          : 'hover:bg-stone-100/70 dark:hover:bg-slate-800/70 border border-transparent'
      }`}
      onClick={() => selectNoteWithConfirmation(note)}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2 min-w-0 flex-1">
          <FileText className={`w-4 h-4 ${selectedNote?.id === note.id ? 'text-blue-500 dark:text-blue-400' : 'text-stone-400 dark:text-slate-400'} flex-shrink-0`} />
          {editingNoteId === note.id ? (
            <div className="flex items-center space-x-2 flex-1 note-edit-container">
              <input
                type="text"
                ...
                className="flex-1 px-2 py-1 text-sm bg-white dark:bg-slate-800 border rounded focus:ring-1 focus:ring-blue-500/50 dark:focus:ring-blue-400/50 outline-none transition-all duration-200"
                autoFocus
              />
              <button
                onClick={(e) => {
            ...
            <div className="flex items-center justify-between flex-1">
              <span className={`truncate text-sm font-medium ${selectedNote?.id === note.id ? 'text-blue-700 dark:text-blue-300' : 'text-stone-700 dark:text-slate-200'}`}>
                {note.title}
              </span>
              <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <button
                  onClick={(e) => startEditingTitle(note, e)}
                  className="p-1 hover:bg-stone-200/70 dark:hover:bg-slate-700/70 rounded-full transition-colors duration-200"
                >
                ...
                <button
                  ...
                  className="p-1 hover:bg-red-100/70 dark:hover:bg-red-900/30 rounded-full text-red-500 dark:text-red-400 transition-colors duration-200"
                >
      ...
      <div className="mt-1 flex justify-between items-center">
        <span className="text-xs text-stone-400 dark:text-slate-400 flex items-center">
          <Clock className="h-3 w-3 mr-1 opacity-70" />
          {formatTimeOnly(note.created_at)}
        </span>
        {note.content && (
          <span className="text-xs text-stone-400 dark:text-slate-400">
    ...
    <>
      ...
      <div className="w-full h-screen overflow-hidden relative bg-card">
        <ResizablePanelGroup direction="horizontal" className="h-full">
          {/* 主侧边栏：笔记列表 */}
          <ResizablePanel defaultSize={20} minSize={15} maxSize={30} className="h-full border-r bg-background/60 backdrop-blur-md">
            <div className="flex flex-col h-full">
              ...
              {/* 切换类型Tab */}
              <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "titles" | "time")} className="px-4 py-3 border-b">
                    ...
                    {activeTab === "titles" ? (
                      ...
                      filteredAndSortedNotes.map((note) => (
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2 min-w-0 flex-1">
                              <FileText className={`w-4 h-4 ${selectedNote?.id === note.id ? 'text-primary' : 'text-muted-foreground'} flex-shrink-0`} />
                              {editingNoteId === note.id ? (
                                <div className="flex items-center space-x-2 flex-1 note-edit-container">
                                  <input
                                    type="text"
                                ...
                                <div className="flex items-center justify-between flex-1">
                                  <span className={`truncate text-sm font-medium ${selectedNote?.id === note.id ? 'text-foreground' : 'text-foreground'}`}>
                                    {note.title}
                                  </span>
                                  <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                    <button
          ...
          <ResizablePanel defaultSize={80} className="h-full bg-background">
            {selectedNote ? (
              <div className="flex flex-col h-full p-6">
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex-1 overflow-auto relative bg-card border rounded-xl shadow-sm">
                    <QuillEditorWrapper
                      ...
                      onHeadingsExtracted={setDocumentHeadings}
                ...
                {/* 底部工具栏 */}
                <div className="flex justify-between items-center mt-4 px-4 py-3 bg-muted/30 border rounded-xl">
