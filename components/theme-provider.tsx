...
export function ThemeProvider({ children, ...props }: ThemeProviderProps) {
  ...
  const handleColorChange = React.useCallback((
    ...
    // 将颜色引用到一个临时变量
    const customColorsValue = customColorsRef.current;
    
    // 只有当新值不同于当前值时才更新状态，避免无限循环
    setCustomColors(prev => {
      // 如果新值与当前值相同，直接返回之前的状态对象，不触发更新
      if (prev[colorType] === value) {
        return prev;
      }
      // 否则返回更新后的状态对象
      return { ...prev, [colorType]: value };
    });
    
    if (!applyImmediately) return;
    
    const currentThemeMode = getThemeMode();
    const newColors = { ...customColorsValue, [colorType]: value };
                ...
                <div className="color-item">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        if (customColors.primary !== e.target.value) {
                          handleColorChange('primary', e.target.value)
                        }
                      }}
                      onBlur={(e) => {
                        if (customColors.primary !== e.target.value) {
                          handleColorChange('primary', e.target.value, true)
                        }
                      }}
                    />
                  </div>
                ...
                <div className="color-item">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        if (customColors.secondary !== e.target.value) {
                          handleColorChange('secondary', e.target.value)
                        }
                      }}
                      onBlur={(e) => {
                        if (customColors.secondary !== e.target.value) {
                          handleColorChange('secondary', e.target.value, true)
                        }
                      }}
                    />
                  </div>
                ...
                <div className="color-item">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        if (customColors.background !== e.target.value) {
                          handleColorChange('background', e.target.value)
                        }
                      }}
                      onBlur={(e) => {
                        if (customColors.background !== e.target.value) {
                          handleColorChange('background', e.target.value, true)
                        }
                      }}
                    />
                  </div>
